    ...
    def read_from(
        ...
        with open(filename, "r", encoding=self.__encoding) as file:
            ...
            self.__data = _imat_type(
                (item.get(self.__data_column, "") for item in csv_reader), include
            )

    @property
    def data(self) -> IMaterials:
        if self.__data is None:
            raise UnfilledAttrException(
                "FileSchema.data is not set! Use await FileSchema.read_from before accessing this attribute"
            )
        return self.__data
